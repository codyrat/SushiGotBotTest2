...
NORMAL_RESTOCK_TIME = 7     # the number of seconds it takes to restock inventory after ordering it
...
def getGameRegion():
    """
    Obtains the region that the Sushi Go Round game is on the screen and assigns it to GAME_REGION.
...
def setupCoordinates():
    ...
    global NORMAL_DELIVERY_BUTTON_COORDS, MAT_COORDS, LEVEL
...
def navigateStartGameMenu():
    """
    Performs the clicks to navigate form the start screen (where the PLAY button is visible)
    to the beginning of the first level.
    """
...
def startServing():
    """
    The main game playing function. This function handles all aspects of game play,
    including identifying orders, making orders, buying ingredients and other features.
    """
...
def clickOnPlates():
    """
    Clicks the mouse on the six places where finished plates will be flashing.
    This function does not check for flashing plates, but simply clicks on all six places.
...
def getOrdersDifference(newOrders, oldOrders):
    ...
    The first dictionary is the "added" dictionary of orders added to newOrders since oldOrders.
...
def makeOrder(orderType):
    ...
    The orderType parameter has the value of one of the
    ONIGIRI,
    GUNKAN_MAKI,
    CALIFORNIA_ROLL,
    SALMON_ROLL,
    ...
    The return value is None for a successfully made order, or the string of an ingredient constant
    if that needed ingredient is missing.
    """
    global ROLLING_COMPLETE, INGRED_COORDS, INVENTORY

    """
    wait until the mat is clear. The previous order could still be there if the conveyor belt has been full or 
    the mat is currently rolling.
    """
    while time.time() < ROLLING_COMPLETE and pyautogui.locateOnScreen(
            imPath('clear_mat.png'), region=(GAME_REGION[0] + 115, GAME_REGION[1] + 295, 220, 175)
    ) is None:
        time.sleep(0.15)
    ...
    print("Makeing order: {}".format(orderType))
    for ingredient, amount in RECIPE[orderType].items():
        print("Need {1} of {0} ingredient".format(ingredient, amount))
        for i in range(amount):
            print("Clicking for {} instance {}".format(ingredient, i))
            pyautogui.click(INGRED_COORDS[ingredient], duration=0.25)
            INVENTORY[ingredient] -= 1
    findAndClickPlatesOnBelt()  # get rid of any left over meals on the conveyor belt, which may stall this meal
    pyautogui.click(MAT_COORDS, duration=0.25)  # click the rolling mat to make the order
    logging.debug('Made a %s order.' % orderType)
    ROLLING_COMPLETE = time.time() + 1.5  # give the mat enough time (1.5 seconds) to finish rolling before being used


def findAndClickPlatesOnBelt():
    """
    Find any plates on the conveyor belt that can be removed and click on them to remove them.
    This will get rid of excess orders.
    """
    for color in ('pink', 'blue', 'red'):
        result = pyautogui.locateCenterOnScreen(imPath('%s_plate_color.png' % color),
                                                region=(GAME_REGION[0] + 343, GAME_REGION[1] + 300, 50, 80))
        if result is not None:
...
def orderIngredient(ingredient):
    """
    Do the clicks to purchase an ingredient. If successful, the ORDERING_COMPLETE dictionary is updated
    for when the ingredients will arrive and INVENTORY can be updated.
    (This is handled in the updateInventory() function.)
    """
